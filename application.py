...
def buy():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO histories (symbol, shares, price, id) VALUES(:symbol, :shares, :price, :id)", symbol = stock["symbol"], shares = shares, price = usd(stock["price"]), id = session["user_id"] )
         ...
         # update user cash    
        db.execute("UPDATE users SET cash = cash - :purchase where id = :id",purchase = stock["price"] * float(shares), id = session["user_id"])
        ...
        if not user_shares:
            db.execute("INSERT INTO portfolio (name, shares, price, total, symbol, id) VALUES( :name, :shares, :price, :total, :symbol, :id)", name=stock["name"], shares=shares, price=usd(stock["price"]), total=usd(stock["price"]*shares), symbol=stock["symbol"], id=session["user_id"])
            
        #elseincrement share count
        else:
...
def sell():
    ...
    if request.method == "GET":
        ...
        #no of shares owned
        shares_owned = db.execute("SELECT shares FROM portfolio where id = :id AND symbol = :symbol", id = session["user_id"], symbol =stock["symbol"])
        
        #check if enough shares
        if not shares_owned or shares > int(shares_owned[0]["shares"]):
        ...
        db.execute("INSERT INTO histories (symbol, shares, price, id) VALUES(:symbol, :shares, :price, :id)", symbol = stock["symbol"], shares = -shares, price = usd(stock["price"]), id = session["user_id"] )
         ...
         # update user's cash(increse)    
        db.execute("UPDATE users SET cash = cash + :selling_price where id = :id",selling_price = stock["price"] * float(shares), id = session["user_id"])
        
        #decrement shares
        shares_total = shares_owned[0]["shares"] - shares
        
        #delete if shares are 0
        if shares_total == 0:
            db.execute("DELETE FROM portfolio WHERE id = :id AND symbol = :symbol",id = session["user_id"], symbol = stock["symbol"])
        else:
            #decrement the number of shares by updating portfolio
            db.execute("UPDATE portfolio SET shares = :shares WHERE id = :id AND symbol = :symbol", shares = shares_total, id = session["user_id"], symbol = stock["symbol"])
...
def loan():
    ...
    if request.method == "GET":
        ...
        try:
            loan = int(request.form.get("amount"))
            if loan < 0:
                return apology("Loan must be +ve")
            elif loan > 1000:
                return apology("Can't give more than $1,000")
        except:
            return apology("Loan must be postive")
